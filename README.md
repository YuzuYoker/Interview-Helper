...
- 🎯 **混合检索**：向量检索（BGE 语义）+ BM25 关键词，RRF 融合，Reranker 精排 —— 检索 Hit@1 **92%** / Hit@3 **98%**（50 题评估集，BM25+RRF+Reranker 配置）
...
- 🚀 **性能优化**：Redis 回答缓存（检索时间从 ~400ms 降至 <20ms，加速 20x）、异步向量化上传（202 立即返回）、滑动窗口限流
...
| 场景 | 数值 |
|---|---|
| 检索 Hit@1 / Hit@3 | **92% / 98%**（50 题评估集，BM25+RRF+Reranker 配置） |
| 检索延迟（无缓存） | 0.38s（纯向量/BM25+RRF）/ 4.75s（+Reranker）/ 10.93s（+HyDE） |
| 缓存命中回答 | **<10ms**（Redis GET 平均 0.19ms，SET 平均 0.26ms） |
| 流式首内容（sources 事件） | <0.5s |
| LLM 首字 | 1-3s |
| 多轮改写命中率 | 80%（5 题中 4 题命中） |
